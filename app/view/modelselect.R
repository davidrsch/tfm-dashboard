...
server <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # inputs
    observeEvent(
      c(input$modelselect,input$daterange1),{
        ...
        output$returnschart <- plotly::renderPlotly({
          ...
          fig <- fig |>
            ...
            add_lines(x=~Date, y =~max_y, name="RNA", line= list(color="blue"),legendgroup = 'struct1') |>
            add_lines(x=~Date, y =~max_95, name="max95", line= list(color="blue"), fill = 'tonexty', fillcolor='rgba(18,18,255,0.2)',legendgroup = 'struct1', showlegend=F) |>
            add_lines(x=~Date, y =~min_5, name="min5", line= list(color="blue"), fill = 'tonexty', fillcolor='rgba(18,18,255,0.5)',legendgroup = 'struct1', showlegend=F) |>
            add_lines(x=~Date, y =~min_y, name="min", line= list(color="blue"), fill = 'tonexty', fillcolor='rgba(18,18,255,0.2)',legendgroup = 'struct1', showlegend=F) |>
            add_lines(x=~Date, y =~meanPortre, name="meanP", line= list(color="blue",dash = 'dash'),legendgroup = 'struct1', showlegend=F) |>
            add_lines(x=~Date, y =~IBEX, name="IBEX", line= list(color="red")) |>
            add_lines(x=~Date, y =~Means, name="Means", line= list(color="green")) |>
            layout(
              title=paste0("Carteras Estructura-",input$modelselect),
              showlegend=T,
              legend = list(y = 0.5),
        ...
        output$indicatorschart <- plotly::renderPlotly({
          ...
          fig <- fig |> 
            add_lines(x=~Date, y =~meanmse, name="MSE", line= list(color="blue")) |>
            add_lines(x=~Date, y =~meanrsqrd, name="R2", line= list(color="green")) |>
            layout(
              title=paste0("Indicadores Estructura-",input$modelselect),
              showlegend=T,
              legend = list(y = 0.5),
              xaxis=list(title="Fecha"),
              yaxis=list(title="Valores")
